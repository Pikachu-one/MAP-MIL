...
if __name__ == "__main__":
    ...
    # ---- 1. 读取配置并实例化模型 ----
    with open('/mnt/a-3/liuzequn/82/home/liuzequn/multi_instance_learning_code/ACMIL-main/config/camelyon_natural_supervised_config.yml', "r") as ymlfile:
        c = yaml.load(ymlfile, Loader=yaml.FullLoader)
        conf = Struct(**c)
        
    x = torch.rand(1, 12255, 1024).cuda()  # 模拟输入
    model = CLAM_MB(input_dim=1024,n_classes=2,dropout=0.25,act='relu').cuda()
    model.eval()
