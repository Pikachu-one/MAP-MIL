...
def save_model(conf, epoch, k,model, optimizer, base_ckpt_dir,is_best_auc=False, is_best_loss=False, is_last=False):
    to_save = {
    ...
    checkpoint_path = os.path.join(base_ckpt_dir, 'checkpoint-%s.pth' % epoch)

    # record the checkpoint with best validation accuracy
    if is_best_auc:
        checkpoint_path = os.path.join(base_ckpt_dir, f'checkpoint-best-{k}-auc.pth')
    if is_best_loss:
        checkpoint_path = os.path.join(base_ckpt_dir, f'checkpoint-best-{k}-loss.pth')
    if is_last:
        checkpoint_path = os.path.join(base_ckpt_dir, f'checkpoint-{k}-last.pth')
